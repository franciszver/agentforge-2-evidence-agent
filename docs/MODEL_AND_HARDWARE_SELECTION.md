...
- **Status:** Final for Phase 2's evidence-retrieval answer path. Documents how the local answer model and its serving configuration were chosen for the reference/minimum hardware tier, and why the verified-citation ceiling that choice produces is **6/12** — not a defect to be tuned away, but the honest capability boundary of an 8 GB GPU at this model class.
- **Method — two different measurements, not one, stated plainly so neither is mistaken for the other:**
  1. **The exploratory hardware-selection sweep** (below, "The ladder actually tested"): a throwaway measurement harness run once per config, best-of-attempts, used only to choose *which* model/config to adopt. Its 8B-Q5 row (4/12) is the number that motivated the selection at the time.
  2. **The committed, CI-reproducible production baseline** (below, "The committed production baseline"): the chosen 8B-Q5 config's citation reliability as measured through the REAL eval pipeline (`evals/cases/citation_present/`) — production `LlamaServerClient`, single-run frozen recordings committed to the repo, fail-closed verification — the number `pytest evals/` reproduces on every run and the P3G.2 gate now guards. That number is **6/12**, one honest single-draw recording (the model has real run-to-run variance; this is not a cherry-picked best case).
  All citation counts use the same fail-closed rule: a citation counts only when a surviving claim carries a verbatim-verified guideline-chunk document citation.
...
The chosen row's exploratory figure (4/12) is superseded by the committed, CI-reproducible production baseline below — **6/12**, measured through the real eval pipeline rather than the throwaway sweep harness. The two numbers aren't contradictory: the sweep was a quick best-of-attempts comparison used only to rank the four candidates against each other, and it correctly ranked 8B-Q5 above 8B-Q4 and 4B; the production baseline is the number this project actually publishes and enforces going forward.
...
Once 8B-Q5 was selected, its real citation reliability was measured the way every other eval category in this project is measured: the 12 `citation_present` eval cases (`evals/cases/citation_present/`) run through the actual production pipeline (planner → claim extraction → fail-closed verification), against the production `LlamaServerClient`, with each case's live-model call sequence recorded once and committed to the repo (`evals/recordings/`) so the result replays byte-for-byte offline in CI. This is the number `pytest evals/` reproduces on every run, and the number `evals/category_baseline.json`'s P3G.2 gate guards against regression.

**Result: 6 of 12 cases pass** (verdict `verified` with a real, verbatim-verified `guideline_chunk` citation). One case's live run could not even be recorded under this engine (a separate, reported extraction-pipeline gap, out of scope here) and is carried forward unchanged; the remaining 5 fail for documented, case-specific reasons (see each case's `xfail` rationale) — two of them are near-misses where the guideline citation itself verifies correctly but an unrelated chart-data claim fails verification and holds the overall verdict at `partially_verified`.

This is a single honest draw, not a best-of-N: the model has real run-to-run variance (the exploratory sweep and this baseline used the same config and still landed on different numbers), so 6/12 should be read as "what one clean, fail-closed run against the production pipeline produces today," not as a tight bound on the model's ceiling in either direction.
...
| Verified citations | 6/12 (measured, committed production baseline) | Expected higher — not measured, since no recommended-tier hardware exists in this project; stated as directional only, per the same discipline `docs/W2_ARCHITECTURE.md` applies to its own projected numbers |
