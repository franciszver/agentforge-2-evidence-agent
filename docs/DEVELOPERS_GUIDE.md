...
- **Stack up:** see `CONTRIBUTING.md` quick start plus the Co-Pilot overlay, from `docker/development-easy/`. First time only: copy `.env.example` to `.env` and fill in `TRACE_ARGS_HASH_SECRET` (generation command in that file) -- the overlay's `agent` service requires it (#180). Then: `docker compose -f docker-compose.yml -f docker-compose.copilot.yml up --build` -- `--build` is required (not just `up`): any machine with a previously built `agent` image won't rebuild it on plain `up`, and only a rebuilt image has `/data/traces` (the persistent trace-store mount, #180) correctly owned for `appuser` to write to.
