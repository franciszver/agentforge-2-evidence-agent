--
-- Clinical Co-Pilot module upgrade 1.0.0 -> 1.0.1 (#124 Phase 2b).
--
-- Adds the per-user OAuth token table to deployments installed before it
-- existed in table.sql. Guarded with #IfNotTable so it is a no-op where the
-- table is already present (idempotent, safe to re-run). Body is identical to
-- table.sql's CREATE TABLE so a fresh install and an upgrade converge on the
-- same schema. Processed by SQLUpgradeService::upgradeFromSqlFile, the same
-- service that runs table.sql on install.
--
